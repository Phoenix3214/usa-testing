...
class World {
    ...
    setCellAsBoosting(cell) {
        ...
        this.boostingCells.unshift(cell);
        return true;
    }
    /** @param {Cell} cell */
    ...
    resolveRigidCheck(a, b) {
      ...
      /*  a.x -= (dx * m * aM) / 1;
        ...
        b.y += (dy * m * .5) / 10;*/
   //   if ( d <= m*m) {  //return
        a.x -= ~~~~~~~~~~~~~~~~~~~~(dx * m * aM) / 1;  // new physics? bounce force
        a.y -= ~~~~~~~~~~~~~~~~~~~~(dy * m * aM) / 1;
        b.x += ~~~~~~~~~~~~~~~~~~~~(dx * m * 0.5) / 10;
        b.y += ~~~~~~~~~~~~~~~~~~~~(dy * m * 0.5) / 10;
     // }
  ...
  */
    ...
    /** @param {Cell} cell */
    boostCell(cell) {
        const d = cell.boost.d / 14 * this.handle.stepMult; //speed/velo of split cells
...
*/
   ...
   bounceCell(cell, bounce) {
    ...
    launchPlayerCell(cell, size, boost) {
        ...
        const x = cell.x + this.settings.playerSplitDistance * boost.dx; // set number to 75 solotrick
        const y = cell.y + this.settings.playerSplitDistance * boost.dy; // set number to 75
    ...
    ejectFromPlayer(player) {
        ...
        for (let i = 0; i < l; i++) {
            ...
            const sx = cell.x + dx * cell.size * 1.1;
            const sy = cell.y + dy * cell.size * 1.1;
      ...
      linePlayerCell(cell, size, boost) {
        cell.squareSize -= size * size;
        const x = cell.x + 0 * boost.dx; // set number to 75 solotrick
        const y = cell.y + 0 * boost.dy; // set number to 75
        const newCell = new PlayerCell(cell.owner, x, y, size);
        newCell.boost.dx = boost.dx;
        newCell.boost.dy = boost.dy;
        newCell.boost.d = boost.d;
    ...
    popPlayerCell(cell) {
        ...
        for (let i = 0, l = splits.length; i < l; i++) {
            const angle = Math.random() * 2 * Math.PI;
            this.linePlayerCell(cell, Math.sqrt(splits[i] * 100), {
                dx: Math.sin(angle),
                dy: Math.cos(angle),
                d: 0//this.settings.playerSplitBoost
            });
        }
    }
